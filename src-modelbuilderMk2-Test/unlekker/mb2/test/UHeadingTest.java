...
public class UHeadingTest extends PApplet {
  UNav3D nav;
...
//    exit();
  ...
  private void reinit() {
    ...
    RotationOrder rotOrder=
        Heading.rotOrders.get(ORDCNT%Heading.rotOrders.size());
    println(rotOrder.toString());
    for(UVertex hv:cpD) head.add(
        new Heading(hv,rotOrder));
    
    bezvl=new ArrayList<UVertexList>();
    int id=0;
    vl=UVertexList.circle(50, 12);
    
   
    for(Heading hh:head) {
      vl2=hh.applyInverse(vl.copy());
      float m=bezierPoint(
          0.05f, 2, 1, 0.2f, map(id,0,head.size()-1,0,1));
...
//      }
    ...
    cp=UCurve.bezier(cp.toArray(), 60).center();
  }
...
//    if(rot==null) return;
    ...
    fill(255);
    text("Heading vector damping: "+nf(TT,0,2)+
        " Order: "+head.get(0).rotOrderName
        ,10,15);
    text(this.getClass().getSimpleName(),10,height-5);
    ...
    int id=0; 
    for(UVertex vv:cp) {
      tmp.set(cpD.get(id++)).norm(50).add(vv);
      UMB.pline(vv,tmp);
    }
    UMB.ppop();
...
//      l.draw();
  ...
  public void keyPressed() {
    if(key==' ') drawGeo=!drawGeo;
      ...
      if(key=='-') TCNT=TCNT>0 ? TCNT-1 : 99;
      else if(key=='+') TCNT=TCNT<99 ? TCNT+1 : 0;
      else if(key=='r') ORDCNT++;
      else cp=null;
      reinit();
    }
  }
